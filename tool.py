...
def class_mmd(s_n, data_s, data_t):
    loss = 0
    mse = nn.MSELoss()
    for j in range(0, s_n):
        # y_prso = torch.tensor(np.zeros((1,s_n)),dtype=torch.float32)
        yt_prso = 0
        ys_prso = 0
        yt = 0
        ys = 0
        ...
        for i in range(0, data_t.shape[0]):
            yt_ij = data_t[i, j]
            yt_prso += yt_ij * data_t[i, :]
            yt += yt_ij

        for i in range(0, data_s.shape[0]):
            ys_ij = data_s[i, j]
            ys_prso += ys_ij * data_s[i, :]
            ys += ys_ij
        loss += mse(yt_prso / yt, ys_prso / ys)

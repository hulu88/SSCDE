...
class MMD_loss(nn.Module):
    ...
    def conditional(self, source, target, s_label, t_label, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
        ...
        # print((2 * weight_st * ST).shape)
        loss += torch.sum(torch.sum(weight_ss * SS) + torch.sum(weight_tt * TT ) - torch.sum(2 * weight_st * ST))
        # print(torch.sum(torch.sum(weight_ss * SS) + torch.sum(weight_tt * TT ) - torch.sum(2 * weight_st * ST)))
        # loss += torch.mean(torch.mean(weight_ss * SS) + torch.mean(weight_tt * TT) - torch.mean(2 * weight_st * ST))
        # print(torch.mean(torch.mean(weight_ss * SS) + torch.mean(weight_tt * TT) - torch.mean(2 * weight_st * ST)))
        # exit()
        return loss